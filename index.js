 ...
 * @author Developer
 * @license MIT
 * @version 1.3.1 (с запоминанием места, без HTML)
 */
...
// ========================================================
// 📆 ТАБЛИЦА НА МЕСЯЦ (чистый текст, узкая)
// ========================================================
...
bot.start((ctx) => {
  ...
  return ctx.reply(text, mainMenu).catch(console.error);
});
...
bot.command('help', (ctx) => {
  ...
  return ctx.reply(text).catch(console.error);
});
...
bot.command('stats', (ctx) => {
  const lastId = getLastLocation(ctx.from.id);
  const location = lastId
    ? [...citiesAreasData.cities, ...citiesAreasData.areas].find(l => l.id == lastId)
    : null;
  ...
  if (location) {
    text += `📍 Последнее место: ${location.name_cities || location.name_areas}`;
  }
...
bot.on('callback_query', async (ctx) => {
  ...
  try {
    ...
    if (data.startsWith('loc_')) {
      ...
      setLastLocation(userId, id); // ✅ Запоминаем место
      return await ctx.editMessageText(
        `📍 ${name}\nВыберите период:`,
        getLocationMenu(id)
      );
